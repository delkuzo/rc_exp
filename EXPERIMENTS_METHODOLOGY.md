...
### Варианты выбора даты (зафиксировано)

Имеем ДВА эталонных варианта селектора даты — оба реализованы и поддерживаются:

1) Ограниченный период — список месяцев
- Контроль: `select-dropdown` со списком месяцев (и годом справа вторичным цветом)
- Назначение: быстрый выбор периода/месяца без точной даты
- Где смотреть: первый ряд в `experiments/button-group-filters.html` (Вариант 1)

2) Полный выбор даты — встроенный календарь
- Контроль: `select-dropdown`, внутри которого отрисован календарь (шапка месяц/год, сетка дней)
- Навигация: стрелки «влево/вправо» для месяца и года (выпадающих списков внутри не используем)
- Выбор: клик по дню подставляет формат `DD Месяц YYYY` в кнопку и закрывает dropdown
- Где смотреть: второй ряд в `experiments/button-group-filters.html` (Вариант 2)

Правило применения:
- Когда нужен «период/месяц» — использовать список месяцев (вариант 1)
- Когда нужна конкретная календарная дата — использовать календарь (вариант 2)
